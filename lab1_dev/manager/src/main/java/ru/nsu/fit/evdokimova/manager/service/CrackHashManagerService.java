...
public class CrackHashManagerService {
    ...
    public ResponseForCrackToClient createCrackRequest(RequestForCrackFromClient request) {
        ...
        requestStorage.put(requestId, new CrackRequestData(StatusWork.IN_PROGRESS, new CopyOnWriteArrayList<>(), System.currentTimeMillis(), 0));

        // **Сразу отдаем requestId клиенту**
    ...
    private void processCrackRequest(String requestId, RequestForCrackFromClient request) {
        ...
        // **Обновляем количество ожидаемых частей**
...
//        if (requestData.getData().size() >= requestData.getExpectedParts()) {
        if (requestData.getData().size() >= requestData.getExpectedParts() && requestData.getStatus() == StatusWork.IN_PROGRESS) {

            requestData.setStatus(StatusWork.READY);
            log.info("Запрос {} завершён", response.getRequestId());
        }
    }
